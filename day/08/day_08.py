...
def get_antena_coordinates(antena_map: np.array, empty_spots: str = ".") -> list[tuple]:
    coordinates = []
...
def get_antinode_coords(antena_map: np.array, firs_coord: tuple, second_cord: tuple) -> list[tuple]:
    valid_coords = []
...
def get_all_antinode_coords(antena_map: np.array, antena_coordinates: list[tuple]) -> set:
    valid_coords = set()
...
def place_antinodes(antena_map: np.array, antena_coordinates: list[tuple], antinode: str = "#", empty_spot: str = "."):
    # We compare each antena with each other
    for i in range(len(antena_coordinates) - 1):
        first_coord: tuple = antena_coordinates[i]
        for j in range(i + 1, len(antena_coordinates)):
...
def count_antinodes(antena_map: np.array, antinode: str = "#") -> int:
    counter = 0
